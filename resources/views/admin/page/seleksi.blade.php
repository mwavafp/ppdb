            ...
            <div class="max-w-7xl my-4">
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
                    ...
                    <div class="flex mt-4 mx-4">
                        <button type="submit"
                            class="bg-green-500 text-white py-2 px-4 rounded-md mr-2 w-[100px] border hover:bg-green-600">Cari</button>
                        <a href="{{ route('seleksi.filter') }}"
                            class="bg-red-500 text-white py-2 px-4 rounded-md w-[100px] border hover:bg-red-600 text-center">Reset</a>
                    </div>
                </div>
            </div>
        </form>
        ...
        <div>
            <table class="min-w-full divide-y divide-gray-200" id="dataTable">
                <thead class="bg-[oklch(62.7%_0.194_149.214)] text-white border-b-2">
                    <tr>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase">No</th>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase">Nama</th>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase">NISN</th>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase">Jenjang</th>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase">Kelas</th>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase">Berkas</th>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase">Status DP</th>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase">Status Calon</th>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase">Status Seleksi</th>
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="border px-4 py-2">
                                @foreach (['kk' => 'Kartu Keluarga', 'pas_foto' => 'Pas Foto', 'ijazah_akhir' => 'Ijazah Akhir', 'kip' => 'KIP'] as $key => $label)
                                    <p class="{{ $student->{'status_' . $key} === 'diserahkan' ? 'bg-green-500' : 'bg-red-500' }} text-white rounded-lg px-2 text-center my-2">
                                        {{ $label }}
                                    </p>
                                @endforeach
                            </td>
                            ...
                            <td class="border px-4 py-2 text-center">
                                <span class="{{
                                    $student->status_seleksi === 'LOLOS' ? 'bg-[oklch(62.7%_0.194_149.214)]' :
                                    ($student->status_seleksi === 'PENDING' ? 'bg-blue-500' : 'bg-red-500')
                                ...
                                <div x-data="{ open: false }">
                                    ...
                                    <div x-show="open"
                                        ...
                                        <div class="bg-white rounded-lg shadow-lg w-11/12 md:w-1/2 p-6 relative">
                                            ...
                                            <form action="{{ route('seleksi.update', $student->id_user) }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <select name="status"
                                                        class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                                        <option value="aktif" {{ $student->status_user == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                                        <option value="tidak_aktif" {{ $student->status_user == 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                                                    </select>
                                                </div>
                                                ...
                                                @foreach (['kk' => 'KK', 'ijazah_akhir' => 'Ijazah Terakhir', 'pas_foto' => 'Pas Foto', 'kip' => 'KIP'] as $field => $label)
                                                    <div class="mb-4">
                                                        <label class="block mb-1 font-medium">Status {{ $label }}</label>
                                                        <select name="status_{{ $field }}"
                                                            class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                                            <option value="diserahkan" {{ $student->{'status_'.$field} == 'diserahkan' ? 'selected' : '' }}>Diserahkan</option>
                                                            <option value="belum_diserahkan" {{ $student->{'status_'.$field} == 'belum_diserahkan' ? 'selected' : '' }}>Belum Diserahkan</option>
                                                        </select>
                                                    </div>
                                                @endforeach
